...
/**
 * COMPLETE ATOMIC SWAP (CORRECTED)
 * 
 * Completes the atomic swap by revealing the secret on both chains
 * using the correct withdraw function for Stellar HTLC.
 */
...
class AtomicSwapCompleter {
  ...
  constructor() {
    ...
    console.log('🔗 COMPLETING ATOMIC SWAP (CORRECTED)');
  ...
  /**
   * Step 2: Withdraw from Stellar HTLC (CORRECTED)
   */
  async withdrawFromStellar(): Promise<void> {
    console.log('\n🌟 Step 2: Withdrawing from Stellar HTLC...');
  ...
  async completeAtomicSwap(): Promise<void> {
    ...
    try {
      ...
      // Step 2: Withdraw from Stellar (CORRECTED)
